import 'package:flutter/cupertino.dart';

class AppColors {
  static const Color primaryColor = Color(0xFF4BB5D9);
  static const Color primaryLightColor = Color(0xFFD1EDF7);
  static const Color secondaryColor = Color(0xFFff9900);
  // static const Color secondaryColor = Color(0xFF4BB5D9);
  static const Color textGreyColor = Color(0xFFCCC7C5);
  static const Color textColor = Color(0xFF23365E);
  static const Color titleColor = Color(0xFF5C524F);
  static const Color paraColor = Color(0xFF8F837F);

  static const Color yellowColor = Color(0xFFFF9900);
  static const Color blackColor = Color(0xFF332D2B);
  static const Color greenColor = Color(0xFF32CF7A);
  static const Color lightGreenColor = Color(0xFFD6F5E4);
  static const Color redColor = Color(0xFFFCAB88);
  static const Color red_2Color = Color(0xFFFF0202);

  // static const Color bgLightColor = Color(0xFFF1F2F6);
  static const Color bgLightColor = CupertinoColors.systemGroupedBackground;
  static const Color boxShadowColor = Color(0xFFE8E8E8);
  static const Color cardBorderColor = Color.fromARGB(255, 172, 172, 172);

  static const Color grey2Color = Color(0xFFBDC1C6);
  static const Color whiteLightColor = Color(0xFFD5DDF2);
  static const Color yellow_2Color = Color(0xFFFF9500);
  static const Color greyColor = Color(0xFF8E8E93);
  static const Color lightGreyColor = Color(0xFFF0F0F0);
  static const Color purpleColor = Color(0xff9f85ff);
  static const Color whiteLight2Color = Color(0xFFF3F6F7);
}
